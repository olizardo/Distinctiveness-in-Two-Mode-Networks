...
Now consider a matrix $\mathbf{B}$ of the same dimensions as $\mathbf{A}$ but with reverse entries: For each cell $b_{ij}$ in $\mathbf{B}$, whenever $a_{ij} = 1$ in $\mathbf{A}$, $b_{ij} = 0$. In the same way, whenever $a_{ij} = 0$ in $\mathbf{A}$, $b_{ij} = 1$ in $\mathbf{B}$. The $\mathbf{B}$ matrix can be considered $\mathbf{A}$'s evil twin (alternatively, $\mathbf{B}$ can be thought of as ``bizarro $\mathbf{A}$'' if you are into Superman comics). For people, $\mathbf{B}$ records \textit{disaffiliations}; the groups each person does not belong to. Similarly, for groups, $\mathbf{B}$ records \textit{non-memberships}, the people who don't belong to each group. 
...
Matrix $\mathbf{P}$ is a $|P| \times |P|$ matrix (with $|P|$ being the cardinality of the person set), with entries $p_{ij}$ equal to the group overlap between persons $i$ and $j$---equivalent to the number of memberships held in common between the two people. Matrix $G$ is a $|G| \times |G|$ matrix (with $|G|$ being the cardinality of the group set), with entries $g_{kl}$ equal to the membership overlap between groups---the number of people held in common by two groups. Both $\mathbf{P}$ and $\mathbf{G}$ can thus be thought of as (weighted) one-mode \textit{similarity} networks recording the pairwise affinities between persons and groups respectively \citep{borgatti1997network}. The frequencies recorded in each cell in the one-mode projection matrices also provide the raw materials to compute alternative pairwise similarity metrics between people or groups, such as Jaccard, Dice, Cosine, min/max overlap, and others \citep{latapy2008basic}. 
...
What's the difference between $\mathbf{P}$ and $\mathbf{P^*}$? Well, since $\mathbf{P}$ is built from information in the original affiliation matrix $\mathbf{A}$, the one mode projection $\mathbf{P}$ counts membership \textit{matches} between people. Matrix $\mathbf{P^*}$ on the other hand, counts \textit{mismatches}; that is, its entries $p^*_{ij}$ record the number of groups the $i^{th}$ person belongs that the $j^{th}$ person does \textit{not} belong to. In this way, $\mathbf{P^*}$ indexes the \textit{distinctiveness} of $i$'s memberships compared to $j$'s for all pairs of people $i$ and $j$. In the same way, for groups, the entries $g^*_{kl}$ in $\mathbf{G^*}$ counts the number of people that belong to the $k^{th}$ group who do not belong to the $l^{th}$ one. In this way, $\mathbf{P^*}$ it records the \textit{distinctiveness} of group $k$'s members compared to $l$'s. 
...
Overall, for each pair of people $i$ and $j$, $i$'s memberships are maximally distinct from $j$'s when there is no overlap between them (and $\hat{p}_{ij} = 1)$. In the same way, $i$'s memberships are minimally distinct from $j$'s when $i$'s memberships are a strict subset of $j$'s (and $\hat{p}_{ij} = 0)$. The same reasoning applies to groups.
...
\begin{equation}
     C^{D}_g = \frac{1}{|G|}\sum_k \hat{g}_{kl}
\end{equation}

Where everything is as defined before. For both persons and groups, $C^{D}$ approaches one as the person's memberships are increasingly distinct from other people's and as the group's members are distinct from other groups. Of course, the value of one can only be reached in the (odd) isolated case of a person who belongs to a ``group'' no one else belongs to. The singleton group containing one member who is not a member of other groups would also achieve the maximum score. Obviously, these limiting cases are of little substantive significance. In the same way, $C^{D}$ approaches zero when a person's memberships are a subset of other people's or a group's members are a subset of that of other groups. The cases toward the lower end of Distinctiveness Centrality are not uninteresting oddities, as it is perfectly possible for a person's memberships to be a strict subset of every other person's, and the same goes for groups. 
